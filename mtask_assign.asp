...
<table border="0" cellpadding="3" cellspacing="0">
  <form action="mtask_assignindb.asp" method="post" name="form_assign" id="form_assign" onsubmit="return(checksubinf(this));">
	...
	select case rs("mjxx") & rs("rwlr")
		...
		if isnull(rs("mtbomr")) and not isnull(rs("mtgyshr")) and isnull(rs("dxbomr")) and not isnull(rs("dxgyshr")) then sel_opt("开始全套BOM")
		if isnull(rs("mtbomr")) and not isnull(rs("mtgyshr")) then sel_opt("开始模头BOM")
		if isnull(rs("dxbomr")) and not isnull(rs("dxgyshr")) then sel_opt("开始定型BOM")
