...
Call ChkPageAble(0)
Call ChkDepart("技术部")
CurPage="分值统计 → 查看统计"					'页面的名称位置( 任务书管理 → 添加任务书)
...
'dtend=cdate(iyear&"年"&imonth&"月5日")
'dtstart=dateadd("m", -1, dtend)		'上个月
...
Function ygkpstatDisplay()
	...
	Select Case ChkJs(tmpAble)
		...
		Case 4	'组长
			...
			<%Call diskpItemM("设计原因产生返修",6.0,2.0, tmpGroup, 2)%>
			<%Call diskpItemM("设计原因产生报废",8.0,4.0, tmpGroup, 2)%>
			<%Call diskpItemM("线切割成型1:1图",4.0,2.0, tmpGroup, 2)%>
		...
		Case 5	'组员
			...
			<tr>
				<td class=ctd><%=icount%></td>
				<td class=ctd>技术标准</td>
				<td class=ctd>技术文件签署、更改不完善</td>
				<td class=ctd>4.0</td>
				<td class=ctd>1.5</td>
				<td class=ctd>分/次</td>
				<%
				...
				<td class=ctd><%=ics(0)%></td>
				<td class=ctd><%=kpif(0)%></td>
				<td class=ctd><%=kpf(3)%></td>
			</tr>

			<%icount=icount+1%>
			<tr>
				<td class=ctd><%=icount%></td>
				<td class=ctd rowspan=4>设计质量</td>
				<td class=ctd>标准结构物料未充分利用</td>
				<td class=ctd rowspan=4>22.0</td>
				<td class=ctd>2.0</td>
				<td class=ctd>分/次</td>
				<%
			...
			<tr>
				<td class=ctd><%=icount%></td>
				<td class=ctd rowspan=2>设计验证</td>
				<td class=ctd>第一次样品合格</td>
				<td class=ctd rowspan=2>&nbsp;</td>
				<td class=ctd>0.08</td>
				<td class=ctd>×模具分值</td>
				<%
					...
					kpf(5)=kpif(0) + kpif(1)
					'If kpf(5)<-10 Then kpf(5)=-10
				%>
				<td class=ctd><%=ics(0)%></td>
				<td class=ctd><%=kpif(0)%></td>
				<td class=ctd rowspan=2><%=kpf(5)%></td>
			</tr>
			<%icount=icount+1%>
			<tr>
				<td class=ctd><%=icount%></td>
				<td class=ctd>第二次样品合格</td>
				<td class=ctd>0.05</td>
				<td class=ctd>×模具分值</td>
				<td class=ctd><%=ics(1)%></td>
				<td class=ctd><%=kpif(1)%></td>
			</tr>
	...
	%>
			<tr>
				...
				<%
					...
					'tmpkpf=statkpfz(arg1, arg4)
					tmpkpf=tmpcs*arg3*-1
					kpf(icount-1)=tmpkpf
					'If kpf(0)>9 Then kpf(1)=9
